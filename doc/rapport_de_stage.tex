...
\end{itemize}
	...
	Lorsqu'un ID erroné est fourni par l'utilisateur, l'API retourne une réponse 404. Voici le traitement effectué dans le code :
	...
	\begin{lstlisting}[language=java]
		try {
			...
			.entity("Forfait avec ID '" + id + "' non trouve")
			.type(MediaType.TEXT_PLAIN)
			.build();
		}
	\end{lstlisting}
	
	Dans cet extrait, un bloc \texttt{try-catch} est utilisé pour capturer une exception si l'ID fourni n'existe pas dans la base de données. Si l'ID est invalide, l'API lance une erreur 404 et renvoie un message texte indiquant que le forfait avec l'ID spécifié n'a pas été trouvé.
	...
	Ce test vérifie que l'endpoint \texttt{/offres/forfait-m} renvoie 4 forfaits et que le premier a l'ID "forfait-m-1".
	...
	Ce test vérifie que l'endpoint \texttt{/offres/forfait-m/{id}} renvoie le forfait avec l'ID "forfait-m-1" et un prix de 1000.0.
	...
	En générant la documentation statique avec Redocly à partir de ces annotations OpenAPI, nous avons pu obtenir une documentation claire et bien structurée : 
