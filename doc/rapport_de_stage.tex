...
\end{itemize}
	...
	\begin{tikzpicture}
		\begin{axis}[
			ybar=0pt,
			bar width=15pt,
			symbolic x coords={Java, Linux, Conteneurs, Bases de données, API REST, LaTeX},
			xtick=data,
			...
			\addplot coordinates {(Java, 2) (Linux, 2) (Conteneurs, 1) (Bases de données, 3) (API REST, 0) (LaTeX, 0)};
			
			% Données Après le stage (à déterminer, on les met en blanc pour indication)
			\addplot coordinates {(Java, 0) (Linux, 0) (Conteneurs, 0) (Bases de données, 0) (API REST, 0) (LaTeX, 0)};
			
			\legend{Avant le stage, Après le stage (à déterminer)}
		\end{axis}
	\end{tikzpicture}
